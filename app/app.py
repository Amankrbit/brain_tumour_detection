...
<style>
    ...
    h2, h3, h4 { color: #f8fafc !important; font-weight: 600 !important; }
    [data-testid="stImage"] {
        border-radius: 12px;
    ...
    [data-testid="stImage"]:hover { transform: scale(1.02); }
    ...
    [data-testid="stChatInput"] textarea { color: #ffffff !important; caret-color: #38bdf8 !important; }
    [data-testid="stChatInputSubmit"] svg { fill: #38bdf8 !important; }
    ...
    .stAlert { border-radius: 12px; backdrop-filter: blur(5px); }
    ...
    if uploaded_file:
        ...
        with st.spinner("Analyzing scan..."):
            ...
            if label == "No_tumor":
                ...
                region_text = "Pituitary Gland Region"
            else:
                region_text = "Cerebral Tissue Region"
        ...
        # --- 7. GENERATE THE CLINICAL REPORT SUMMARY ---
        st.markdown("---")
        ...
        **Model Confidence:** {conf:.2f}%
        ...
        # Detailed format exactly as requested by the user
        st.markdown("#### 🔬 Detailed MRI Findings (AI-Generated Template)")
        st.info(f"""
        **1. Procedure Description:** MRI brain performed with and without contrast. Agent used: Gadolinium-based (Assumed standard protocol).

        **2. Findings:** * **Location:** {region_text} (Localized via Grad-CAM algorithmic mapping).
        * **Size:** Precise volumetric extraction requires 3D radiologist software. Refer to AI Bounding Box for 2D spatial estimation.
        * **Signal Characteristics:** Pending human radiologist review (e.g., T1 hypointense, T2 hyperintense).
        * **Enhancement Pattern:** Pending post-contrast visual assessment (e.g., Heterogeneous vs. Ring-enhancing).
        * **Edema:** AI screening indicates focus on primary mass; perilesional edema must be manually assessed.
        * **Mass Effect & Diffusion Restriction:** Requires certified clinical review.

        **3. Impression (Conclusion):** * Findings highly suggestive of neoplastic lesion.
        * **Likely Diagnosis:** {label}.
        * **Next Steps:** MRI does NOT confirm cancer definitively. Final confirmation must be done by Biopsy (gold standard) and Histopathology.
        """)
        ...
        # ALL code below this point is now properly indented to be INSIDE the "if uploaded_file:" block
        ...
        # --- 9. AI MEDICAL ASSISTANT CHATBOT (GROQ) ---
        st.markdown("---")
